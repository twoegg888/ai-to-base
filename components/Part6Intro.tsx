...
interface Part6IntroProps {
  onComplete: () => void;
}

export function Part6Intro({ onComplete }: Part6IntroProps) {
  ...
  useEffect(() => {
    const part6Timer = setTimeout(() => setShowPart6(true), 300);
    const titleTimer = setTimeout(() => setShowTitle(true), 1000);
    return () => {
      clearTimeout(part6Timer);
    ...
    <div className="bg-white relative size-full">
      ...
      <div className="absolute flex flex-col font-['Pretendard:SemiBold',_sans-serif] justify-center leading-[normal] not-italic text-[#102a71] text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-center top-1/2 tracking-[-1.5px] translate-x-[-50%] translate-y-[-50%]" style={{ left: "calc(50% + 0.5px)" }}>
        {showTitle && (
          ...
          >
            <TypewriterText text="환경적 요인과" speed={100} delay={0} className="mb-0 block whitespace-nowrap" />
            <TypewriterText text="스트레스는?" speed={100} delay={800} className="block whitespace-nowrap" />
          </motion.div>
        )}
      </div>
    </div>
