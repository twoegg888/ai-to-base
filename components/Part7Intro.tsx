...
interface Part7IntroProps {
  onComplete: () => void;
}

export function Part7Intro({ onComplete }: Part7IntroProps) {
  ...
  useEffect(() => {
    const part7Timer = setTimeout(() => setShowPart7(true), 300);
    const titleTimer = setTimeout(() => setShowTitle(true), 1000);
    return () => {
      clearTimeout(part7Timer);
    ...
    <div className="bg-white relative size-full">
      ...
      <div className="absolute flex flex-col font-['Pretendard:SemiBold',_sans-serif] justify-center leading-[normal] not-italic text-[#102a71] text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-center top-1/2 tracking-[-1.5px] translate-x-[-50%] translate-y-[-50%]" style={{ left: "calc(50% + 0.5px)" }}>
        {showTitle && (
          ...
          >
            <TypewriterText text="개인 선호도와" speed={100} delay={0} className="mb-0 block whitespace-nowrap" />
            <TypewriterText text="목표는?" speed={100} delay={800} className="block whitespace-nowrap" />
          </motion.div>
        )}
      </div>
    </div>
