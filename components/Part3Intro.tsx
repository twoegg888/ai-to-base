...
interface Part3IntroProps {
  onComplete: () => void;
}

export function Part3Intro({ onComplete }: Part3IntroProps) {
  ...
  useEffect(() => {
    const part3Timer = setTimeout(() => setShowPart3(true), 300);
    ...
    <div className="bg-white relative size-full">
      ...
      <div className="absolute flex flex-col font-['Pretendard:SemiBold',_sans-serif] justify-center leading-[normal] not-italic text-[#102a71] text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-center top-1/2 tracking-[-1.5px] translate-x-[-50%] translate-y-[-50%]" style={{ left: "calc(50% + 0.5px)" }}>
        {showTitle && (
          ...
          >
            <TypewriterText text="생활 습관과" speed={100} delay={0} className="mb-0 block whitespace-nowrap" />
            <TypewriterText text="라이프스타일은?" speed={100} delay={800} className="block whitespace-nowrap" />
          </motion.div>
        )}
      </div>
    </div>
